...
export const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        const response = await fetch('http://4.233.138.141:3001/api/user/profile', {
          method: 'GET',
          headers: {
    ...
    <main className="container mx-auto px-10 sm:px-12 py-36">
      <div className=" bg-white shadow-lg rounded-lg p-8 mb-12 lg:mx-28">
        <div className="flex flex-col sm:flex-row items-center">
          <img src={user.profileImage} alt="Profile" className="w-40 rounded-full object-cover mb-4 sm:mr-10 shadow-md" />
            ...
            <div class="container flex flex-col sm:flex-row items-center justify-between">
              ...
              <div className="text-right flex flex-col items-center sm:items-end mb-4">
                <div className="flex items-center sm:justify-end mb">
                  <span className="text-yellow-400 ml-2">{'★'.repeat(Math.floor(user.rating))}</span>
                  <span className="text-gray-400">{'★'.repeat(5 - Math.floor(user.rating))}</span>
                </div>
                <a href={user.reviewsLink} className="hover:underline text-gold">Toutes les évaluations</a>
              </div>
            </div>
