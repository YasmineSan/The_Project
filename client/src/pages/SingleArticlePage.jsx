...
const SingleArticlePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOneArticle = async () => {
      try {
        const response = await fetch(`https://167.172.38.235:3001/api/articles/article/${articleId}`, {
          method: 'GET',
          headers: {
    ...
    const fetchArticles = async (userId) => {
      try {
        const response = await fetch(`https://167.172.38.235:3001/api/articles/user/${userId}/articles`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
  ...
  const handleAddToFavorites = async () => {
    try {
      const response = await fetch('https://167.172.38.235:3001/api/favorite', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
